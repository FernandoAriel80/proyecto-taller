        ...
        <div class="grid md:grid-cols-2 rounded-md gap-2 p-1 row-span-1 bg-white ">
            <section class="w-full max-w-full overflow-hidden">
                ...
                <article class="grid grid-cols-2 gap-3">
                    ...
                    <p class="break-words overflow-auto">{{ $assigned_employee->vehicleInWorkshop->name }}</p>
            
                    <label class="font-bold truncate" for="">Correo:</label>
                    <p class="break-words overflow-auto">{{ $assigned_employee->vehicleInWorkshop->email }}</p>
            
                    <label class="font-bold truncate" for="">Teléfono:</label>
                    <p class="break-words overflow-auto">{{ $assigned_employee->vehicleInWorkshop->phone_number }}</p>
                </article>
            </section>
            
            <section class="w-full max-w-full overflow-hidden">
                <h4 class="flex font-bold mb-4 bg-slate-500 text-white p-1 rounded-tl rounded-tr">Detalles del vehiculo:</h4>
                <article class="grid grid-cols-2 gap-3">
                    <label class=" font-bold truncate" for="">Vehiclulo:</label>
                    <p class="break-words overflow-auto">{{ $assigned_employee->vehicleInWorkshop->vehicle_type }}</p>
                    <label class=" font-bold truncate" for="">Marca:</label>
                    <p class="break-words overflow-auto">{{ $assigned_employee->vehicleInWorkshop->brand }}</p>
                    <label class=" font-bold truncate" for="">Patente:</label>
                    <p class="break-words overflow-auto">{{ $assigned_employee->vehicleInWorkshop->license_plate }}</p>
                    <label class=" font-bold truncate" for="">Del año:</label>
                    <p class="break-words overflow-auto">{{ $assigned_employee->vehicleInWorkshop->year }}</p>
                </article>
            </section>
        </div>
        <div class=" rounded-md p-1 bg-white  row-start-2">
            <h4 class="flex font-bold mb-4 bg-slate-500 text-white p-1 rounded-tl rounded-tr">Descripción del problema:</h4>
            <article>
                <p>{{ $assigned_employee->vehicleInWorkshop->description }}</p>
            </article>
        </div>
        <div class=" rounded-md p-1 bg-white  row-span-2">
            <h4 class="flex font-bold mb-4 bg-slate-500 text-white p-1 rounded-tl rounded-tr">Detalles del presupuesto actual:</h4>
            <article>
